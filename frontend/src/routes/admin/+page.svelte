...
<div class="grid gap-4 lg:grid-cols-2">
  <section class="card p-4 text-sm lg:col-span-2">
    ...
    <p class="mb-2 text-xs text-slate-500">بات را به گروه اضافه کنید تا اینجا نمایش داده شود؛ سپس گروه را به یک پروژه وصل کنید.</p>
    {#each groups as g (g.id)}
      <div class="flex flex-wrap items-center gap-2 border-t border-slate-100 py-2 dark:border-slate-800">
        <span class="font-medium">{g.title}</span><span class="text-xs text-slate-400" dir="ltr">{g.chatId}</span>
    ...
    {/each}
    {#if !groups.length}<div class="p-4 text-center text-slate-400">هنوز گروهی ثبت نشده است.</div>{/if}
    {#if logs}
      <div class="mt-3 max-h-64 overflow-auto rounded-lg bg-slate-50 p-2 text-xs dark:bg-slate-800/50">
        {#each logs.list as m}<div class="py-0.5"><b>{m.fromName}:</b> {m.text} <span class="text-slate-400">{fmtDateTime(m.createdAt)}</span></div>{/each}
  ...
  <section class="card space-y-2 p-4 text-sm">
    ...
    {#each users as u (u.id)}
      <div class="flex items-center gap-2 border-t border-slate-100 py-1.5 dark:border-slate-800 {u.active ? '' : 'opacity-50'}">
        <span>{u.name}</span><span class="text-xs text-slate-400">@{u.username} · {SYS_ROLES[u.role]}</span>
