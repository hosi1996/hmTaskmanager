...
{:else}
  <div class="card relative mb-5 overflow-hidden p-5">
    ...
    <div class="flex flex-wrap items-center gap-4">
      <span class="tile h-14 w-14" style="background:{project.color}18;color:{project.color}"><Icon name={project.icon} size={26} /></span>
      <div class="min-w-0 flex-1">
        <h1 class="h-page truncate">{project.name}</h1>
        <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
          <span class="chip bg-zinc-100 dark:bg-white/[0.06]">{ROLES[role]}</span>
  ...
  <div class="seg mb-6 max-w-full overflow-x-auto">
    {#each tabs as [k, l, ic]}
      <button class="seg-btn !px-4 !py-2 !text-[13px] {tab === k ? 'seg-btn-on' : ''}" onclick={() => openTab(k)}><Icon name={ic} size={15} />{l}</button>
    {/each}
  </div>
  ...
  {:else if tab === 'overview'}
    <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
      {#each [['کل', data.stats.total, ''], ['باز', data.stats.open, ''], ['انجام‌شده', data.stats.closed, 'text-emerald-600'], ['عقب‌افتاده', data.stats.overdue, 'text-red-600']] as [l, v, c]}
        <div class="card p-3"><div class="text-xs text-zinc-500">{l}</div><div class="text-2xl font-bold {c}">{num(v)}</div></div>
      {/each}
    </div>
    <div class="card mb-4 p-4">
      <div class="mb-2 text-sm text-zinc-500">پیشرفت: {data.stats.total ? num(Math.round((data.stats.closed / data.stats.total) * 100)) : '۰'}٪ {#if project.deadline}· ددلاین: {fmtDate(project.deadline)}{/if}</div>
      <div class="h-2 overflow-hidden rounded-full bg-zinc-100 dark:bg-white/[0.06]"><div class="h-full bg-emerald-500" style="width:{data.stats.total ? (data.stats.closed / data.stats.total) * 100 : 0}%"></div></div>
    </div>
    {#if workload.length}
      <div class="card mb-4 p-4"><div class="mb-2 font-semibold">بار کاری اعضا</div>
        {#each workload as w}<div class="mb-1 flex items-center gap-2 text-sm"><span class="w-32 truncate">{w.user?.name}</span><div class="h-2 flex-1 rounded bg-zinc-100 dark:bg-white/[0.06]"><div class="h-full rounded bg-brand" style="width:{Math.min(100, w.open * 10)}%"></div></div><span>{num(w.open)}</span></div>{/each}
      </div>
    {/if}
    <div class="card p-4"><div class="mb-2 font-semibold">فعالیت‌های اخیر</div>
      {#each data.recent as a}<div class="border-b border-zinc-100 py-1.5 text-sm last:border-0 dark:border-white/[0.08]">{a.user?.name ?? 'سیستم'} {ACTIONS[a.action] ?? a.action}{#if a.task} <a class="text-brand" href="/tasks/{a.taskId}">#{a.task.number}</a>{/if}<span class="ms-2 text-xs text-zinc-400">{fmtDateTime(a.createdAt)}</span></div>{/each}
      {#if !data.recent.length}<div class="text-sm text-zinc-400">فعالیتی ثبت نشده.</div>{/if}
    </div>
  {:else if tab === 'members'}
    <div class="card divide-y divide-zinc-100 dark:divide-white/[0.06]">
      {#each project.members as m (m.userId)}
        <div class="flex items-center gap-3 p-3 text-sm">
          <span class="font-medium">{m.user.name}</span><span class="text-xs text-zinc-400">@{m.user.username} · {SYS_ROLES[m.user.role]}</span>
          {#if isManager}
          ...
          {:else}<span class="chip ms-auto bg-zinc-100 dark:bg-white/[0.06]">{ROLES[m.role]}</span>{/if}
        </div>
      {/each}
    </div>
    {#if isManager}
      <div class="card mt-4 space-y-3 p-4">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <button class="btn-ghost" onclick={invite}><Icon name="link" size={15} /> لینک دعوت مشتری</button>
        </div>
        {#each newMember.found as u}<button class="btn-ghost w-full justify-start" onclick={() => setRole(u.id, newMember.role)}><Icon name="user-plus" size={15} /> {u.name} (@{u.username})</button>{/each}
